...
public class Main {
    public static void main(String[] args) {
        ...
        Preprocessor preprocessor = new JPreprocessor();
        Compiler compiler = new JCompiler();
        Linker linker = new JLinker();
        ...
        ide.build(code).execute();

        System.out.println("\nIDE에서 소스 코드 리스트를 이용해서 빌드 후 실행");
        ide.createProject("ListProject");
        ide.build(Arrays.asList(code)).execute();
    }
}
